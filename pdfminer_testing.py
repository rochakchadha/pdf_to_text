...
def convert_pdf_to_text_clean_split(input_path, page_length=512, overlap=20):
    output_string = StringIO()
    ...
    # convert each page to utf-8 encoding and remove the null characters, remove the lines with only 1 character or with just a number like 60.3
    for i in range(len(file_name_list)):
        file_name_list[i] = file_name_list[i].encode('utf-8').decode('utf-8').replace('\x00','')
        file_name_list[i] = '\n'.join([line for line in file_name_list[i].split('\n') if len(line) > 1])
        file_name_list[i] = '\n'.join([line for line in file_name_list[i].split('\n') if not line.replace('.','',1).isdigit()])
    
    # TODO: recursively split the pages into smaller pages. Each page should be upto page_length words long. The split pages should overlap by overlap words.
